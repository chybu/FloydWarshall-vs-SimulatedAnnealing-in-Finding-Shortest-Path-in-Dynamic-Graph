...
def getCost_check(solution:list, graph:list, temp:float, maxWei=2000):
    base_penalty = maxWei*100  # this can be tuned 
    penalty = int(round(base_penalty / (temp**0.5 + 1)))
    print(penalty, temp)
    cost = 0
    for i in range(len(solution) - 1):
    ...
    for i in range(1, max_iter+1):
        ...
        if delta < 0 or random() < math.exp(-delta / T):
            ...
            if no_improvement_for > 150:
                T *= 1.2  # reheat
                no_improvement_for = 0           
                # T = T0 / math.log(i + 1)  # log cool
                # T = T0 * (1 - i/max_iter) #linear cool
...
def run_parallel_sa(graph:list, nodes:int, start_solution_path:list, amount:int):
    ...
    # prefs_insert_random =  [1.0, 1.0, 1.0, 1.5, 1.5, 1.2, 1.2, 1.0, 1.0]
...
def convertToVisualizationFormat(path_list:list, id_dict:dict, sa_num)->None:
    visual_list = []
    ...
    joblib.dump(visual_list, f'saved_path_{sa_num}.joblib')

def bfs(graph: list[list[int]], origin: int, target: int) -> list:
    n = len(graph)
    visited = [False] * n
    prev = [None] * n
...
if __name__ == "__main__":    
    ...
    # Change the values to test different case
    nodes  = 50
    dens = 0.3
    
    
    maxWei = 2000 # remember to updates getCost when change max weight    
    graph = generateAdjacencyMatrix(nodes, maxWei, dens) 
    sa_solvers = 4
    fw_wei = maxWei//5
    ...
    if nodes<101:
        ...
        if fw_wei!=-1:
            ...
            optimal_path, optimal_wei = dijkstra(fw_changed_graph, origin, target)
    ...
    if len(start_solution_path)==0:
        ...
        optimal_path, optimal_wei = dijkstra(best_sa_result[3], origin, target)
